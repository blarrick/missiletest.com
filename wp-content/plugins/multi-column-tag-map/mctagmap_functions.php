...
===== */ 
    ...
    $list .= '<!-- begin list -->'."\n".'<div id="mcTagMap" class="'.$equalize.' '.$toggable.' '.$css3.'">'."\n";
	...
	if($taxonomy){
		...
		$tags = get_posts(array('numberposts' => -1, 'sort_order' => 'asc', 'sort_column' => 'post_title', 'post_status' => 'publish', 'post_type' => $type));
	} else {
				...
				if(isset($tag->$arraypart)){
					...
					if ($hide == "yes"){
						...
						if ($i >= $num2show1){
							...
							$list .= '<li><a title="' . $name . '" href="' . $url . '">' . $name .' '.$id.'</a>' . $mctagmap_count . $mctagmap_description;
						}
						if($show_pages == "yes" && $show_child_pages == "yes" && $page_hierarchy == "yes"){
							$list .= '<ul>'.wp_list_pages('echo=0&title_li=&child_of='.$tag->ID).'</ul>';
						}
						$list .= '</li>'."\n";
					}	
				}
